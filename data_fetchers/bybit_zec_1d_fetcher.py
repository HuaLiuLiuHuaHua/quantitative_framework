"""
Bybit ZEC 1D Data Fetcher
Bybit ZEC 日K線數據抓取器

整合設計：
- trading_framework的分塊抓取和錯誤處理
- Medium的數據驗證邏輯
- 自動保存到data/資料夾
"""
...
def fetch_bybit_zec_1d_data(
    start_date: Optional[Union[str, datetime]] = None,
    end_date: Optional[Union[str, datetime]] = None,
    days: int = 365,
    limit: int = 1000,
    sleep_per_request: float = 0.15,
    chunk_days: int = 1000,  # 1D使用1000天分塊：每塊最多1000條日K線數據
    save: bool = True,
    verbose: bool = False,
) -> pd.DataFrame:
    """
    抓取Bybit ZEC/USDT 日K線數據
    ...
    Example:
        >>> # 抓取2020-2024年的ZEC日K數據
        >>> df = fetch_bybit_zec_1d_data(
    ...
    if save:
        ...
        filename = f"ZECUSDT_1d_{start_str}_{end_str}.csv"
...
def _fetch_single_range(
    ...
    try:
        ...
        if all_klines:
            return pd.DataFrame(all_klines).drop_duplicates(subset=["timestamp"])
        else:
