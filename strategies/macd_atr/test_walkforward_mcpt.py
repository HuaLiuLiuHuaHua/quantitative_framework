"""
MACD-ATR Strategy - Walk-Forward MCPT
MACD-ATR策略 - Walk-Forward蒙地卡羅排列測試
...
def run_walkforward_mcpt(
    macd_fast_range: tuple, macd_slow_range: tuple, macd_signal_range: tuple, atr_window_range: tuple, atr_multiplier_range: tuple,
    ...
    real_signals, real_pf = walkforward_optimization(
        df, macd_atr_strategy_func, param_grid, train_window, train_step
    )
    ...
    for perm_i in tqdm(range(n_permutations)):
        try:
            ...
            perm_signals, perm_pf = walkforward_optimization(
                perm_df, macd_atr_strategy_func, param_grid, train_window, train_step
            )
    ...
    result_dirname = f"WalkforwardMCPT_MACD_ATR_BTC1h_{date_str}"
...
if __name__ == "__main__":
    MACD_FAST_RANGE = (8, 16, 2)
    MACD_SLOW_RANGE = (20, 30, 2)
    MACD_SIGNAL_RANGE = (7, 11, 2)
    ...
    try:
        results = run_walkforward_mcpt(
            macd_fast_range=MACD_FAST_RANGE, macd_slow_range=MACD_SLOW_RANGE, macd_signal_range=MACD_SIGNAL_RANGE, atr_window_range=ATR_WINDOW_RANGE, atr_multiplier_range=ATR_MULTIPLIER_RANGE,
            start_date=START_DATE,
            end_date=END_DATE,
            n_permutations=N_PERMUTATIONS,
