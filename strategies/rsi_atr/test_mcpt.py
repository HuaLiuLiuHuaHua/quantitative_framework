"""
RSI-ATR Strategy - Monte Carlo Permutation Test (MCPT)
RSI-ATR策略 - 蒙地卡羅排列測試
...
def run_mcpt(
    ...
    rsi_window: int = 14,
    rsi_long_threshold: int = 60,
    rsi_short_threshold: int = 40,
    atr_window: int = 14,
    ...
    Args:
        ...
        rsi_window: RSI 窗口期
        rsi_long_threshold: RSI 做多閾值
        rsi_short_threshold: RSI 做空閾值
        atr_window: ATR 窗口期
    ...
    if start_index <= max(rsi_window, atr_window):
        raise ValueError(
            f"start_index({start_index})必須大於max(rsi_window, atr_window)({max(rsi_window, atr_window)})，"
            f"以確保有足夠的歷史數據用於指標計算。"
            f"建議: start_index >= max(rsi_window, atr_window) + 10"
        )
    ...
    if rsi_window < 2:
        raise ValueError(f"rsi_window必須大於等於2，當前值: {rsi_window}")
    # 根據數據配置設定年化參數
    periods_per_year = 365 if data_config == "BTCUSDT_1d" else 365 * 24

    if verbose:
        print("\n" + "=" * 70)
        print("RSI-ATR策略MCPT測試")
        print("=" * 70)
        print(f"參數設置:")
        print(f"  數據配置:         {data_config}")
        print(f"  rsi_window:       {rsi_window}")
    ...
    # 驗證數據質量
    if len(df) < start_index + max(rsi_window, atr_window):
        raise ValueError(
            f"數據長度不足: 需要至少 {start_index + max(rsi_window, atr_window)} 根K棒，"
            f"實際只有 {len(df)} 根K棒"
        )
    ...
    if verbose:
        print(f"\n數據驗證通過:")
        print(f"  數據長度:         {len(df)}")
        print(f"  最小要求:         {start_index + max(rsi_window, atr_window)}")
    ...
    try:
        ...
        data_source = f"MCPT_RSI_ATR_{data_config}_rsi{rsi_window}_atr{atr_window}_{date_str}"
    ...
    if save_path:
        ...
        try:
            ...
            plot_mcpt_distribution_medium_style(
                ...
                title=f"MCPT - RSI-ATR Strategy (rsi={rsi_window}, atr={atr_window})",
...
if __name__ == "__main__":
    ...
    # ========== MCPT參數 ==========
    RSI_WINDOW = 4                   # RSI 窗口期
    RSI_LONG_THRESHOLD = 90           # RSI 做多閾值
    RSI_SHORT_THRESHOLD = 5          # RSI 做空閾值
    ATR_WINDOW = 1                   # ATR 窗口期
    ...
    START_INDEX = 5                 # 開始排列的索引
    N_JOBS = -1                        # 並行核心數 (Windows多進程問題，暫時使用1)

    # 執行MCPT測試
    print("\n" + "=" * 70)
    print("RSI-ATR策略 - MCPT測試")
    print("=" * 70)
