"""
LSTM SOL Strategy - Basic Backtest
LSTM SOL策略 - 基礎回測測試

測試目的：
1. 驗證策略基本功能
2. 評估技術指標組合的績效
3. 生成權益曲線圖表
4. 保存回測結果

數據來源：
- Bybit SOL/USDT 1H K線數據
- 自動抓取或從 data/ 載入
"""

import sys
from pathlib import Path
sys.path.append(str(Path(__file__).parents[2]))

import matplotlib
matplotlib.use('Agg')  # 禁用圖片自動顯示

import pandas as pd
import numpy as np
from datetime import datetime

# 導入策略和工具
from strategies.lstm_sol.strategy import LSTMSOLStrategy
from shared.backtest import BacktestEngine
from shared.visualization import plot_backtest_results
from data_fetchers.bybit_sol_1h_fetcher import fetch_bybit_sol_1h_data


def load_or_fetch_data(data_config: str = "SOLUSDT_1h", start_date: str = "2022-10-01", end_date: str = "2024-09-30", verbose: bool = True):
    """
    載入或抓取數據 (優先使用本地數據)

    使用統一的data_loader模組,優先從本地載入已下載的數據,
    大幅提升測試速度,避免重複下載。

    Args:
        data_config: 數據配置 (SOLUSDT_1h 或其他)
        start_date: 開始日期
        end_date: 結束日期
        verbose: 是否打印詳細信息

    Returns:
        pd.DataFrame: OHLCV數據
    """
    from shared.data_loader import load_data_with_fallback

    # 根據配置選擇數據源和fetcher
    if data_config == "SOLUSDT_1h":
        data_source = "1h"
        symbol = "SOLUSDT"
        fetcher_func = fetch_bybit_sol_1h_data
    else:
        raise ValueError(f"不支援的數據配置: {data_config}")

    return load_data_with_fallback(
        data_source=data_source,
        symbol=symbol,
        start_date=start_date,
        end_date=end_date,
        fetcher_func=fetcher_func,
        verbose=verbose
    )


def run_backtest(
    data_config: str = "SOLUSDT_1h",
    symbol: str = "SOL/USD",
    confidence_threshold: float = 0.01,
    look_back: int = 72,
    start_date: str = "2022-10-01",
    end_date: str = "2024-09-30",
    transaction_cost: float = 0.002,
    slippage: float = 0.0005,
    initial_capital: float = 100000,
    verbose: bool = True
):
    """
    執行 LSTM SOL 策略回測

    Args:
        data_config: 數據配置
        symbol: 交易對符號
        confidence_threshold: 置信度閾值 (簡化版未使用)
        look_back: 回溯窗口 (簡化版未使用)
        start_date: 開始日期
        end_date: 結束日期
        transaction_cost: 交易成本（0.002 = 0.2%）
        slippage: 滑價（0.0005 = 0.05%）
        initial_capital: 初始資金
        verbose: 是否打印詳細信息

    Returns:
        Dict: 回測結果字典
    """
    if verbose:
        print("\n" + "=" * 70)
        print("LSTM SOL 策略回測 (簡化版)")
        print("=" * 70)
        print(f"參數設置:")
        print(f"  數據配置:         {data_config}")
        print(f"  交易對:           {symbol}")
        print(f"  置信度閾值:       {confidence_threshold}")
        print(f"  回溯窗口:         {look_back}")
        print(f"  交易成本:         {transaction_cost*100:.2f}%")
        print(f"  滑價:             {slippage*100:.3f}%")
        print(f"  初始資金:         ${initial_capital:,.2f}")
        print("=" * 70)

    # 步驟1: 載入數據
    df = load_or_fetch_data(data_config, start_date, end_date, verbose)

    # 步驟2: 生成策略信號
    if verbose:
        print("\n生成策略信號...")

    strategy = LSTMSOLStrategy()
    signals = strategy.generate_signals(
        df,
        symbol=symbol,
        confidence_threshold=confidence_threshold,
        look_back=look_back
    )

    if verbose:
        print(f"信號統計:")
        print(f"  做多信號: {(signals == 1).sum()}")
        print(f"  做空信號: {(signals == -1).sum()}")
        print(f"  空倉信號: {(signals == 0).sum()}")

    # 步驟3: 執行回測
    if verbose:
        print("\n執行回測...")

    # 根據數據配置設定年化參數
    periods_per_year = 365 * 24  # 1小時線，一年 365*24 小時

    engine = BacktestEngine(
        data=df,
        signals=signals,
        transaction_cost=transaction_cost,
        slippage=slippage,
        initial_capital=initial_capital,
        periods_per_year=periods_per_year
    )

    results = engine.run()

    # 步驟4: 打印績效摘要
    if verbose:
        engine.print_summary()

    # 步驟5: 保存結果
    date_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    data_source = f"Backtest_LSTMSOL_{data_config}_{date_str}"

    # 保存到 strategies/lstm_sol/results/
    save_path = engine.save_results(data_source=data_source, output_dir=Path(__file__).parent / "results")

    # 步驟6: 繪製圖表
    if verbose:
        print("\n繪製權益曲線...")

    plot_save_path = save_path / "backtest_chart.png"
    plot_backtest_results(
        equity_curve=results['equity_curve'],
        data=df,
        signals=signals,
        title=f"LSTM SOL 策略回測 ({symbol}, {data_config})",
        save_path=plot_save_path,
        figsize=(15, 10)
    )

    if verbose:
        print(f"\n圖表已保存: {plot_save_path}")
        print("=" * 70)
        print("回測完成！")
        print("=" * 70)

    return {
        'results': results,
        'strategy': strategy,
        'signals': signals,
        'data': df,
        'save_path': save_path
    }


if __name__ == "__main__":
    # ========== 數據配置 ==========
    DATA_CONFIG = "SOLUSDT_1h"       # 數據配置: SOLUSDT_1h

    # ========== 策略參數 ==========
    SYMBOL = "SOL/USD"               # 交易對符號
    CONFIDENCE_THRESHOLD = 0.01      # 置信度閾值 (簡化版未使用)
    LOOK_BACK = 72                   # 回溯窗口 (簡化版未使用)
    START_DATE = "2022-10-01"        # 開始日期
    END_DATE = "2024-09-30"          # 結束日期
    TRANSACTION_COST = 0.002         # 交易成本 0.2%
    SLIPPAGE = 0.0005                # 滑價 0.05%
    INITIAL_CAPITAL = 100000         # 初始資金 $100,000

    # 執行回測
    print("\n" + "=" * 70)
    print("LSTM SOL 策略 - 基礎回測測試")
    print("=" * 70)

    backtest_results = run_backtest(
        data_config=DATA_CONFIG,
        symbol=SYMBOL,
        confidence_threshold=CONFIDENCE_THRESHOLD,
        look_back=LOOK_BACK,
        start_date=START_DATE,
        end_date=END_DATE,
        transaction_cost=TRANSACTION_COST,
        slippage=SLIPPAGE,
        initial_capital=INITIAL_CAPITAL,
        verbose=True
    )

    print("\n測試完成！")
    print(f"結果已保存到: {backtest_results['save_path']}")
