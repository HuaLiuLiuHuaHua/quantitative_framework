"""
LSTM SOL Strategy - Walk-Forward MCPT
LSTM SOL策略 - Walk-Forward蒙地卡羅排列測試
...
def run_walkforward_mcpt(
    confidence_threshold_range: tuple = (0.01, 0.5, 0.1),
    look_back_range: tuple = (24, 168, 24),
    ...
    Args:
        confidence_threshold_range: 置信度閾值範圍 (start, end, step)
        look_back_range: 回溯窗口範圍 (start, end, step)
        start_date: 開始日期
        end_date: 結束日期
    ...
    print(f"Walk Forward MCPT Analysis - LSTM SOL Strategy")
    ...
    def lstm_sol_strategy_func(data: pd.DataFrame, confidence_threshold: float, look_back: int):
        ...
        signals = strategy.generate_signals(data, symbol="SOL/USD", confidence_threshold=confidence_threshold, look_back=look_back)
        return signals.values  # strategy.generate_signals() returns pd.Series directly

    # 步驟3: 設置參數網格
    param_grid = {
        'confidence_threshold': (confidence_threshold_range[0], confidence_threshold_range[1], confidence_threshold_range[2]),
        'look_back': (look_back_range[0], look_back_range[1], look_back_range[2])
    }
    ...
    real_signals, real_pf = walkforward_optimization(
        df, lstm_sol_strategy_func, param_grid, train_window, train_step
    )
    ...
    for perm_i in tqdm(range(n_permutations)):
        try:
            ...
            perm_signals, perm_pf = walkforward_optimization(
                perm_df, lstm_sol_strategy_func, param_grid, train_window, train_step
            )
    ...
    result_dirname = f"WalkforwardMCPT_LSTMSOL_SOL1h_{date_str}"
    ...
    plt.title(f"Walkforward MCPT - LSTM SOL. P-Value: {walkforward_mcpt_pval:.4f}")
    plt.grid(False)
    plt.legend()
    plt.tight_layout()
...
if __name__ == "__main__":
    # 配置參數
    CONFIDENCE_THRESHOLD_RANGE = (0.01, 0.5, 0.1)  # 置信度閾值從0.01到0.5，步長0.1
    LOOK_BACK_RANGE = (24, 168, 24)   # 回溯窗口從24到168，步長24
    ...
    try:
        results = run_walkforward_mcpt(
            confidence_threshold_range=CONFIDENCE_THRESHOLD_RANGE,
            look_back_range=LOOK_BACK_RANGE,
            start_date=START_DATE,
            end_date=END_DATE,
            n_permutations=N_PERMUTATIONS,
