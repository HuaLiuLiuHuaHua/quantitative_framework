"""
BB-ATR Strategy - Monte Carlo Permutation Test (MCPT)
BB-ATR策略 - 蒙地卡羅排列測試
...
def run_mcpt(
    ...
    if start_index <= max(bb_window, atr_window):
        raise ValueError(
            f"start_index({start_index})必須大於max(bb_window, atr_window)({max(bb_window, atr_window)})，"
            f"以確保有足夠的歷史數據用於指標計算。"
            f"建議: start_index >= max(bb_window, atr_window) + 10"
        )
    ...
    if bb_window < 2:
        raise ValueError(f"bb_window必須大於等於2，當前值: {bb_window}")
    # 根據數據配置設定年化參數
    periods_per_year = 365 if data_config == "BTCUSDT_1d" else 365 * 24

    if verbose:
        print("\n" + "=" * 70)
        print("BB-ATR策略MCPT測試")
        print("=" * 70)
        print(f"參數設置:")
    ...
    # 驗證數據質量
    if len(df) < start_index + max(bb_window, atr_window):
        raise ValueError(
            f"數據長度不足: 需要至少 {start_index + max(bb_window, atr_window)} 根K棒，"
            f"實際只有 {len(df)} 根K棒"
        )
    ...
    if verbose:
        print(f"\n數據驗證通過:")
        print(f"  數據長度:         {len(df)}")
        print(f"  最小要求:         {start_index + max(bb_window, atr_window)}")
    ...
    try:
        ...
        data_source = f"MCPT_BB_ATR_{data_config}_bb{bb_window}_atr{atr_window}_{date_str}"
    ...
    if save_path:
        ...
        try:
            ...
            plot_mcpt_distribution_medium_style(
                ...
                title=f"MCPT - BB-ATR Strategy (bb={bb_window}, atr={atr_window})",
...
if __name__ == "__main__":
    ...
    # ========== MCPT參數 ==========
    BB_WINDOW = 124                         # Bb Window
    BB_STD = 2.5                        # Bb Std
    ATR_WINDOW = 8                         # Atr Window
    ...
    START_INDEX = 125                 # 開始排列的索引
    N_JOBS = -1                        # 並行核心數 (Windows多進程問題，暫時使用1)

    # 執行MCPT測試
    print("\n" + "=" * 70)
    print("BB-ATR策略 - MCPT測試")
    print("=" * 70)
