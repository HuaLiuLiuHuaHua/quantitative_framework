"""
MA Crossover Strategy - Basic Backtest
MA交叉策略 - 基礎回測測試

測試目的：
1. 驗證策略基本功能
2. 評估固定參數(short=20, long=90)的績效
3. 生成權益曲線圖表
4. 保存回測結果
...
def run_backtest(
    ...
    if verbose:
        print("\n" + "=" * 70)
        print("MA Crossover策略回測")
        print("=" * 70)
        print(f"參數設置:")
        print(f"  數據配置:         {data_config}")
        print(f"  short_period:     {short_period}")
        print(f"  long_period:      {long_period}")
    ...
    signals = strategy.generate_signals(df, short_period=short_period, long_period=long_period)
    ...
    data_source = f"Backtest_MACrossover_{data_config}_short{short_period}_long{long_period}_{date_str}"

    # 保存到strategies/ma_crossover/results/
    ...
    plot_backtest_results(
        ...
        data=df,
        signals=signals,
        title=f"MA Crossover策略回測 (short_period={short_period}, long_period={long_period})",
...
if __name__ == "__main__":
    ...
    # ========== 策略參數 ==========
    SHORT_PERIOD = 36                # 短期均線週期 (從優化得到)
    LONG_PERIOD = 518                 # 長期均線週期 (從優化得到)
