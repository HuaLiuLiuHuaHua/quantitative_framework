...
def validate_params(params: dict, df_len: int) -> None:
    ...
    if lookback_period >= df_len:
        errors.append(f"lookback_period ({lookback_period}) >= 數據長度 ({df_len})")
...
def _calculate_signals(momentum_shifted: np.ndarray) -> np.ndarray:
    """
    計算動量信號
...
class MomentumStrategy(BaseStrategy):
    ...
    def generate_signals(self, df: pd.DataFrame, **params) -> pd.Series:
        ...
        # ==================== 計算信號 ====================
        signals_arr = _calculate_signals(momentum_shifted.values)
