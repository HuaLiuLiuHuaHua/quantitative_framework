"""
Donchian Strategy - Basic Backtest
Donchian策略 - 基礎回測測試

測試目的：
1. 驗證策略基本功能
2. 評估固定參數(lookback=20)的績效
3. 生成權益曲線圖表
4. 保存回測結果
...
def run_backtest(
    ...
    signals = strategy.generate_signals(df, lookback=lookback)
    ...
    data_source = f"Backtest_Donchian_{data_config}_lookback{lookback}_{date_str}"

    # 保存到strategies/donchian/results/
    ...
    plot_backtest_results(
        ...
        data=df,
        signals=signals,
        title=f"Donchian策略回測 (lookback={lookback})",
...
if __name__ == "__main__":
    ...
    # ========== 策略參數 ==========
    LOOKBACK = 1440                    # Donchian通道回溯期 (從優化得到)
