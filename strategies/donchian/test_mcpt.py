"""
Donchian Strategy - Monte Carlo Permutation Test (MCPT)
Donchian策略 - 蒙地卡羅排列測試
...
測試模式：
- mode='normal': 使用固定參數(lookback=20)
...
def run_mcpt(
    ...
    Args:
        ...
        lookback: Donchian通道回溯期（normal模式使用）
        start_date: 開始日期
        end_date: 結束日期
        n_permutations: 排列次數
    ...
    if start_index <= lookback:
        raise ValueError(
            f"start_index({start_index})必須大於lookback({lookback})，"
            f"以確保有足夠的歷史數據用於指標計算。"
            f"建議: start_index >= lookback + 10"
        )
    ...
    if lookback < 2:
        raise ValueError(f"lookback必須大於等於2，當前值: {lookback}")
    # 根據數據配置設定年化參數
    periods_per_year = 365 if data_config == "BTCUSDT_1d" else 365 * 24

    if verbose:
        print("\n" + "=" * 70)
        print("Donchian策略MCPT測試")
        print("=" * 70)
        print(f"參數設置:")
    ...
    # 驗證數據質量
    if len(df) < start_index + lookback:
        raise ValueError(
            f"數據長度不足: 需要至少 {start_index + lookback} 根K棒，"
            f"實際只有 {len(df)} 根K棒"
        )
    ...
    if verbose:
        print(f"\n數據驗證通過:")
        print(f"  數據長度:         {len(df)}")
        print(f"  最小要求:         {start_index + lookback}")
    ...
    if mode == 'normal':
        ...
        strategy_params = {'lookback': [lookback]}  # 這裡簡化為單一參數
    ...
    try:
        ...
        # 改進命名：包含測試名稱、策略、數據源、lookback 參數
        data_source = f"MCPT_Donchian_{data_config}_lookback{lookback}_{date_str}"
    ...
    if save_path:
        ...
        try:
            ...
            plot_mcpt_distribution_medium_style(
                ...
                title=f"MCPT - Donchian Strategy (lookback={lookback})",
...
if __name__ == "__main__":
    ...
    # ========== MCPT參數 ==========
    LOOKBACK = 90                     # Donchian通道回溯期 (從優化得到)
