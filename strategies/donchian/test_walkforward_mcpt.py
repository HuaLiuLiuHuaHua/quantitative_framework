"""
Donchian Strategy - Walk-Forward MCPT
Donchian策略 - Walk-Forward蒙地卡羅排列測試
...
def run_walkforward_mcpt(
    lookback_range: tuple = (10, 100, 10),
    ...
    Args:
        lookback_range: lookback參數範圍 (start, end, step)
        start_date: 開始日期
        end_date: 結束日期
    ...
    def donchian_strategy_func(data: pd.DataFrame, lookback: int):
        ...
        signals = strategy.generate_signals(data, lookback=lookback)
        return signals['position'].values
    ...
    real_signals, real_pf = walkforward_optimization(
        df, donchian_strategy_func, param_grid, train_window, train_step
    )
    ...
    for perm_i in tqdm(range(n_permutations)):
        try:
            ...
            perm_signals, perm_pf = walkforward_optimization(
                perm_df, donchian_strategy_func, param_grid, train_window, train_step
            )
    ...
    result_dirname = f"WalkforwardMCPT_Donchian_BTC1h_{date_str}"
...
if __name__ == "__main__":
    # 配置參數
    LOOKBACK_RANGE = (10, 100, 10)    # lookback從10到100，步長10
