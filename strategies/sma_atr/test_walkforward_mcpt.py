"""
SMA-ATR Strategy - Walk-Forward MCPT
SMA-ATR策略 - Walk-Forward蒙地卡羅排列測試
...
def run_walkforward_mcpt(
    sma_period_range: tuple, atr_window_range: tuple, atr_multiplier_range: tuple,
    ...
    real_signals, real_pf = walkforward_optimization(
        df, sma_atr_strategy_func, param_grid, train_window, train_step
    )
    ...
    for perm_i in tqdm(range(n_permutations)):
        try:
            ...
            perm_signals, perm_pf = walkforward_optimization(
                perm_df, sma_atr_strategy_func, param_grid, train_window, train_step
            )
    ...
    result_dirname = f"WalkforwardMCPT_SMA_ATR_BTC1h_{date_str}"
...
if __name__ == "__main__":
    ...
    try:
        results = run_walkforward_mcpt(
            sma_period_range=SMA_PERIOD_RANGE, atr_window_range=ATR_WINDOW_RANGE, atr_multiplier_range=ATR_MULTIPLIER_RANGE,
            start_date=START_DATE,
            end_date=END_DATE,
            n_permutations=N_PERMUTATIONS,
