"""
SMA-ATR Strategy - Monte Carlo Permutation Test (MCPT)
SMA-ATR策略 - 蒙地卡羅排列測試
...
# 導入策略和工具
from strategies.sma_atr.strategy import SMAATRStrategy
from shared.mcpt import MonteCarloTester
from shared.visualization import plot_mcpt_distribution_medium_style


def sma_atr_strategy_func(data: pd.DataFrame, sma_period: int, atr_window: int, atr_multiplier: float):
    """SMA-ATR策略包裝函數 (模組級別函數以支援多進程)"""
    strategy = SMAATRStrategy()
...
def load_data(data_config: str = "BTCUSDT_1d", start_date: str = "2022-10-01", end_date: str = "2024-09-30", verbose: bool = True):
    ...
    # 解析時間框架
    if "_1h" in data_config:
        timeframe = "1h"
    elif "_1d" in data_config:
        timeframe = "1d"
    else:
        raise ValueError(f"Invalid data_config: {data_config}. Must contain '_1h' or '_1d'")

    # 根據配置選擇對應的 fetcher
    if "BTCUSDT" in data_config and timeframe == "1h":
        from data_fetchers.bybit_btc_1h_fetcher import fetch_bybit_btc_1h_data
        data_source = "1h"
        fetcher_func = fetch_bybit_btc_1h_data
    elif "BTCUSDT" in data_config and timeframe == "1d":
        data_source = "1d"
        fetcher_func = fetch_bybit_btc_1d_data
    elif "SOLUSDT" in data_config and timeframe == "1h":
        from data_fetchers.bybit_sol_1h_fetcher import fetch_bybit_sol_1h_data
        data_source = "1h"
        fetcher_func = fetch_bybit_sol_1h_data
    else:
        raise ValueError(f"Unsupported data_config: {data_config}. Supported: BTCUSDT_1d, BTCUSDT_1h, SOLUSDT_1h")
...
def run_mcpt(
    ...
    if start_index <= max(sma_period, atr_window):
        raise ValueError(
            f"start_index({start_index})必須大於max(sma_period, atr_window)({max(sma_period, atr_window)})，"
            f"以確保有足夠的歷史數據用於指標計算。"
            f"建議: start_index >= max(sma_period, atr_window) + 10"
        )
    ...
    if sma_period < 2:
        raise ValueError(f"sma_period必須大於等於2，當前值: {sma_period}")
    # 根據數據配置設定年化參數
    periods_per_year = 365 if "_1d" in data_config else 365 * 24

    if verbose:
        print("\n" + "=" * 70)
        print("SMA-ATR策略MCPT測試")
        print("=" * 70)
        print(f"參數設置:")
    ...
    # 驗證數據質量
    if len(df) < start_index + max(sma_period, atr_window):
        raise ValueError(
            f"數據長度不足: 需要至少 {start_index + max(sma_period, atr_window)} 根K棒，"
            f"實際只有 {len(df)} 根K棒"
        )
    ...
    if verbose:
        print(f"\n數據驗證通過:")
        print(f"  數據長度:         {len(df)}")
        print(f"  最小要求:         {start_index + max(sma_period, atr_window)}")
    ...
    try:
        tester = MonteCarloTester(
            strategy_func=sma_atr_strategy_func,
            data=df,
    ...
    try:
        ...
        data_source = f"MCPT_SMA_ATR_{data_config}_sma{sma_period}_atr{atr_window}_{date_str}"
    ...
    if save_path:
        ...
        try:
            ...
            plot_mcpt_distribution_medium_style(
                ...
                title=f"MCPT - SMA-ATR Strategy (sma={sma_period}, atr={atr_window})",
...
if __name__ == "__main__":
    # ========== 數據配置 ==========
    DATA_CONFIG = "BTCUSDT_1d"        # 數據配置: BTCUSDT_1d 或 BTCUSDT_1h
    ...
    START_INDEX = 100                 # 開始排列的索引
    N_JOBS = 1                        # 並行核心數 (Windows多進程問題，暫時使用1)

    # 執行MCPT測試
    print("\n" + "=" * 70)
    print("SMA-ATR策略 - MCPT測試")
    print("=" * 70)
