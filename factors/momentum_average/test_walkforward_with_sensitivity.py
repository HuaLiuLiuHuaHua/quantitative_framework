"""
Cross-Sectional Moving Average Cross Factor - Walk-Forward Optimization with Sensitivity Analysis
...
from factors.momentum_average.factor import MovingAverageCrossFactor
...
def evaluate_single_params(params, **kwargs):
    ...
    Optimized version: precomputed universe data passed via kwargs
    """
    ...
    try:
        ...
        # Fast fail for invalid parameter combinations
        if short_window >= long_window:
            return {**params, "sharpe_ratio": -99, "turnover": 0, "subniverse_sharpe": -99, "total_return": -1, "max_drawdown": 1}
        ...
        # Use precomputed average universe size (OPTIMIZATION)
        avg_universe_size = kwargs.get("avg_universe_size", 40)
        universe_csv_path = kwargs["universe_csv_path"]

        # VALIDATION: Ensure top_n doesn't exceed available assets
        top_n = percentile_to_n(avg_universe_size, long_short_percentile)
        top_n = max(1, min(top_n, avg_universe_size // 2))  # Cap at half universe size
        bottom_n = top_n

        price_data_cache = kwargs.get("price_data_cache", {})
        factor = MovingAverageCrossFactor()

        backtester = CrossSectionalBacktester(
            factor_calculator=factor,
        ...
        factor_df = factor.calculate(price_data_cache, short_window=short_window, long_window=long_window, min_volume=0)
...
def optimize_window_with_sensitivity(
    ...
    **kwargs
    ...
    # OPTIMIZATION: Precompute universe statistics for this training window
    universe_csv_path = kwargs["universe_csv_path"]
    ...
    universe_df_train = universe_df[
        (universe_df["timestamp"] >= train_start_dt) &
        (universe_df["timestamp"] <= train_end_dt)
    ]
    ...
    logger.info(f"预计算训练窗口平均universe大小: {avg_universe_size}")
    ...
    eval_kwargs = {
        **kwargs,
        ...
        "avg_universe_size": avg_universe_size  # Pass precomputed value
    }
    ...
    logger.info(f"开始敏感性分析前的参数过滤，初始数量: {len(results_df)}")

    # 步骤 1: 筛选 Sharpe Ratio > 阈值的参数
    pre_filtered_df = results_df[results_df["sharpe_ratio"] > sharpe_threshold].copy()
    logger.info(f"Sharpe > {sharpe_threshold} 的参数数量: {len(pre_filtered_df)}")

    # 步骤 2: 在满足Sharpe条件的参数中，筛选换手率
    pre_filtered_df = pre_filtered_df[
        (pre_filtered_df["turnover"] >= 0.01) & (pre_filtered_df["turnover"] <= 0.7)
    ]
    logger.info(f"进一步筛选换手率 (0.01 <= turnover <= 0.7) 后剩余: {len(pre_filtered_df)}")

    logger.info("对筛选后的参数运行敏感性分析")
    robust_params_df, best_robust_params = filter_robust_params(
        results_df=pre_filtered_df, # 使用预筛选后的DataFrame
        param_grid=param_grid,
        metric="sharpe_ratio",
        sharpe_threshold=sharpe_threshold, # 阈值仍然传递，以防内部需要
        top_n=10
    )

    if robust_params_df.empty:
        logger.warning(f"[窗口{window_id}] 没有参数组合满足稳健性标准，使用fallback策略")
        print(f"⚠️  警告: 没有参数组合满足稳健性标准")
        print("回退到初始最佳夏普比率参数")
...
def run_oos_test(
    ...
    param_grid: dict,
    price_data_cache: dict,
    precomputed_universe_stats: dict = None,  # NEW: Accept precomputed stats
    **kwargs
    ...
    try:
        ...
        # OPTIMIZATION: Use precomputed universe stats if available
        if precomputed_universe_stats and window_id in precomputed_universe_stats:
            avg_universe_size = precomputed_universe_stats[window_id]
            logger.debug(f"使用预计算的OOS universe大小: {avg_universe_size}")
        else:
        ...
        oos_metrics = {
            ...
            "total_turnover_value": perf_summary.get('total_turnover_value', 0.0),
            "num_rebalances": perf_summary.get('num_rebalances', 0)
        }
...
def run_walkforward_with_sensitivity(
    ...
    print("="*80)
    print("Cross-Sectional Walk-Forward Optimization with Sensitivity Analysis")
    print("="*80)
    print(f"\n配置:")
    ...
    # OPTIMIZATION: Precompute OOS universe statistics for all windows
    print("\n预计算所有OOS窗口的universe统计信息...")
    precomputed_oos_universe_stats = {}
    for i, window in enumerate(windows):
        test_start = pd.to_datetime(window["test_start"])
        test_end = pd.to_datetime(window["test_end"])
        universe_df_test = universe_df[
            (universe_df["timestamp"] >= test_start) &
            (universe_df["timestamp"] <= test_end)
        ]
        universe_sizes = []
        for _, row in universe_df_test.iterrows():
            symbols = [s for s in row[1:] if pd.notna(s) and s != ""]
            universe_sizes.append(len(symbols))
        precomputed_oos_universe_stats[i] = int(np.mean(universe_sizes)) if universe_sizes else 40
    print(f"完成预计算: {len(precomputed_oos_universe_stats)} 个OOS窗口")
    ...
    for i, window in enumerate(windows):
        ...
        oos_equity, oos_metrics = run_oos_test(
            ...
            param_grid=param_grid,
            price_data_cache=price_data_cache,
            precomputed_universe_stats=precomputed_oos_universe_stats,  # Pass precomputed stats
            **kwargs
        )
        ...
        window_results.append({
            ...
            "total_turnover_value": oos_metrics["total_turnover_value"],
            "num_rebalances": oos_metrics["num_rebalances"],
    ...
    if not results_df.empty:
        ...
        total_oos_turnover_val = results_df["total_turnover_value"].sum()
        total_oos_rebalances = results_df["num_rebalances"].sum()
        total_oos_turnover = total_oos_turnover_val / total_oos_rebalances if total_oos_rebalances > 0 else 0
        ...
        summary = {
            ...
            "total_oos_turnover": float(total_oos_turnover)
        }
    else:
    ...
    try:
        plot_walkforward_performance(
            results_df=results_df,
            title="Walk-Forward OOS Performance vs BTC Benchmark",
    ...
    try:
        ...
        ax.set_title("In-Sample vs Out-of-Sample Sharpe Ratio", fontsize=14, fontweight="bold")
    ...
    print(f"整体OOS换手率: {summary.get('total_oos_turnover', 0):.4f}")
...
if __name__ == "__main__":
    ...
    N_TRIALS = 1000  # Reduced from 10000 for faster execution (parameter space = 13050)
    N_JOBS = -1  # Use all available CPU cores (optimized from 3)
