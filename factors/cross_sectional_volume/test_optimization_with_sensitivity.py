...
from factors.cross_sectional_volume.factor import VolumeFactor
...
def evaluate_single_params(params: dict, **kwargs) -> dict:
    ...
    try:
        ...
        # 初始化負成交量因子
        negative_volume_factor = VolumeFactor()
...
def main():
    ...
    # 參數網格(全部使用range())
    PARAM_GRID = {
        "window": range(32, 61, 1),       # 成交量回看期: 1-450天
        "long_short_percentile": range(10, 60, 10),   # 做多做空百分位: 20%
        "leverage": range(1, 7, 1),                  # 槓桿倍數: 1倍
        "rebalance_freq_days": range(1, 31, 1)       # 調倉頻率: 1天
    }
    ...
    if factor_equity is not None and not factor_equity.empty:
        ...
        if first_trade_date_str is not None:
            first_trade_date = pd.to_datetime(first_trade_date_str)
            factor_equity_aligned = factor_equity[factor_equity.index >= first_trade_date]
            plot_title = f"負成交量因子策略 vs BTC基準 (從首次交易日期 {first_trade_date_str} 開始)"
        else:
            factor_equity_aligned = factor_equity
            plot_title = "負成交量因子策略 vs BTC基準"
