...
class VolumeFactor(BaseFactor):
    """
    A factor that is directly related to trading volume.
    It calculates the rolling average volume.
    The strategy will long assets with high volume and short assets with low volume.
    """
    def calculate(self, price_data: dict, window: int, min_volume: int = 0) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame: A DataFrame with timestamps as index, symbols as columns, and the average volume as values.
        """
        all_factor_values = {}
        for symbol, df in price_data.items():
            if df is not None and not df.empty and 'volume' in df.columns:
                ...
                # The factor is the average volume, shifted to prevent lookahead bias.
                # This way, higher volume gets a higher score.
