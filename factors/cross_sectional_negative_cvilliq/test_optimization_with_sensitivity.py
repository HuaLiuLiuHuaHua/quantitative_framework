...
from factors.cross_sectional_cvilliq.factor import CVILLIQFactor
...
def evaluate_single_params(params: dict, **kwargs) -> dict:
    ...
    try:
        ...
        # 計算因子
        factor_df = cvilliq_factor.calculate(price_data_cache, window=window, min_volume=0)
...
def main():
    """主函數"""
    print("=" * 80)
    print("CVILLIQ 因子市場中性策略 - 參數優化與敏感性分析")
    print("=" * 80)
    print()
    ...
    # 參數網格(全部使用range())
    PARAM_GRID = {
        "window": range(48, 77, 1), # CVILLIQ 窗口: 2-450天 (起始值必須>1)
        "long_short_percentile": range(10, 60, 10),   # 做多做空百分位: 10%-60%
        "leverage": range(1, 7, 1),                  # 槓桿倍數: 1倍
        "rebalance_freq_days": range(1, 31, 1)       # 調倉頻率: 1-30天
    }
    ...
    max_lookback = max(PARAM_GRID["window"]) if "window" in PARAM_GRID and PARAM_GRID["window"] else 0
    available_days = (pd.to_datetime(END_DATE) - pd.to_datetime(START_DATE)).days

    if max_lookback > available_days:
        print(f"警告: 最大window期間 ({max_lookback}天) 超過測試期間 ({available_days}天)")
        print(f"       前{max_lookback}天將作為warmup期（無交易），實際交易期為{available_days - max_lookback}天")
        print(f"       建議: 增加測試期間或減小window參數範圍\n")
    ...
    # 解析最佳參數
    window = int(best_robust_params["window"])
    ...
    print(f"最佳參數: window={window}, percentile={long_short_percentile}%, leverage={leverage}x, rebalance={rebalance_freq_days}D")
    ...
    if factor_equity is not None and not factor_equity.empty:
        ...
        if first_trade_date_str is not None:
            first_trade_date = pd.to_datetime(first_trade_date_str)
            factor_equity_aligned = factor_equity[factor_equity.index >= first_trade_date]
            plot_title = f"CVILLIQ 因子策略 vs BTC基準 (從首次交易日期 {first_trade_date_str} 開始)"
        else:
            factor_equity_aligned = factor_equity
            plot_title = "CVILLIQ 因子策略 vs BTC基準"
