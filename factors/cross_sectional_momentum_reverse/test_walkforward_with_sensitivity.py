"""
Cross-Sectional Momentum Factor - Walk-Forward Optimization with Sensitivity Analysis
...
from factors.cross_sectional_momentum_reverse.factor import NegativeMomentumFactor
...
def evaluate_single_params(params, **kwargs):
    ...
    try:
        ...
        # Initialize momentum_reverse factor
        momentum_reverse_factor = NegativeMomentumFactor()

        # Initialize ExcessReturnPositionStrategy for reversal trading
        position_strategy = ExcessReturnPositionStrategy()

        # Create backtester
        backtester = CrossSectionalBacktester(
            factor_calculator=momentum_reverse_factor,
        ...
        factor_df = momentum_reverse_factor.calculate(price_data_cache, lookback_period=lookback_period, min_volume=0)

        if factor_df is None or factor_df.empty:
            raise ValueError("Factor calculation returned empty or None dataframe.")

        # Run backtest (pass lookback_period for ExcessReturnPositionStrategy)
        results = backtester.run(factor_df=factor_df, lookback_period=lookback_period)
...
def run_oos_test(
    ...
    **kwargs
    ...
    try:
        ...
        # Calculate momentum_reverse factor
        momentum_reverse_factor = NegativeMomentumFactor()
...
if __name__ == "__main__":
    ...
    # Note: Reduced parameter space compared to test_optimization for computational efficiency
    # Each walk-forward window runs independent optimization
    PARAM_GRID = {
        "lookback_period": range(1, 450, 45),  # Momentum lookback: 20-200 days (also used for excess return calculation)
        "rebalance_freq_days": range(1, 151, 15)  # Rebalance frequency: 7-42 days
    }

    # --- Walk-Forward Configuration ---
    TRAIN_DAYS = 780          # Training window: 780 days (~2 years)
    TEST_DAYS = 150        # Testing window: 30 days (~1 month)
    STEP_DAYS = 150         # Step size: 30 days (rolling forward)
