"""
動量反轉因子市場中性策略 - 參數優化與敏感性分析

策略特點:
- 相對回報均值回歸加權法: 所有標的按相對回報（偏離市場平均）動態分配權重
- 關鍵公式: w_i = -(r_i - <r_j>) / Σ|r_k - <r_j>|
  * 超跌標的（負相對回報）→ 多頭
  * 超漲標的（正相對回報）→ 空頭
- 不使用固定top/bottom分組, 避免信號截斷
- 市場中性: 多空權重始終平衡（Σ|w_i| = 1）
...
優化參數:
- lookback_period: 動量回看期 (1-450天)
- rebalance_freq_days: 調倉頻率 (天數)

作者: Claude Code
創建日期: 2025-10-16
最後修改: 2025-11-15 (改為相對回報均值回歸加權)
"""
...
def evaluate_single_params(params: dict, **kwargs) -> dict:
    ...
    try:
        ...
        # 運行回測 (lookback_period 會傳給 ExcessReturnPositionStrategy)
...
def main():
    ...
    # 參數網格(全部使用range())
    PARAM_GRID = {
        "lookback_period": range(1, 450, 45),       # 動量回看期: 1-450天
        "rebalance_freq_days": range(1, 151, 15)       # 調倉頻率: 1-150天
    }
    ...
    # Debug mode
    DEBUG_MODE = True  # 設置為True以啟用詳細日誌輸出
    ...
    max_lookback = max(PARAM_GRID["lookback_period"]) if "lookback_period" in PARAM_GRID and PARAM_GRID["lookback_period"] else 0
    ...
    print(f"最佳參數: lookback={lookback_period}, rebalance={rebalance_freq_days}D")
    print(f"持倉模式: 相對回報均值回歸加權")
    print(f"  超跌標的（負相對回報）→ 多頭")
    print(f"  超漲標的（正相對回報）→ 空頭")
    print(f"  權重: w_i = -(r_i - <r_j>) / Σ|r_k - <r_j>|\n")
    ...
    # 運行最終回測
    print("\n開始運行回測...")
    final_results = backtester.run(factor_df=factor_df, lookback_period=lookback_period)
    ...
    best_params_summary = {
        ...
        "configuration": {
            "strategy_type": "relative_return_mean_reversion",
            "position_weighting": "dynamic (all assets by relative return to market avg)",
            "weight_formula": "w_i = -(r_i - <r_j>) / Σ|r_k - <r_j>|",
            "start_date": START_DATE,
            "end_date": END_DATE,
            "sharpe_threshold": SHARPE_THRESHOLD
        }
    ...
    if factor_equity is not None and not factor_equity.empty:
        ...
        if first_trade_date_str is not None:
            first_trade_date = pd.to_datetime(first_trade_date_str)
            factor_equity_aligned = factor_equity[factor_equity.index >= first_trade_date]
            plot_title = f"動量因子策略 vs BTC基準 (從首次交易日期 {first_trade_date_str} 開始)"
        else:
            factor_equity_aligned = factor_equity
            plot_title = "動量因子策略 vs BTC基準"
