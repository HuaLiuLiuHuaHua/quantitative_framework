...
def evaluate_single_params(params, **kwargs):
    ...
    try:
        ...
        holding_days = int(params.get("holding_days", rebalance_freq_days))  # Default to rebalance_freq_days if not provided
        ...
        # Create backtester with RotatingPositionStrategy
        ...
        # Run backtest with holding_days and rebalance_freq_days (matching Optimization file)
...
def optimize_window_with_sensitivity(
    ...
    **kwargs
    ...
    if robust_params_df.empty:
        logger.warning(f"[窗口{window_id}] 没有参数组合满足 Sharpe > {sharpe_threshold} 且換手率 {turnover_min}~{turnover_max} 的条件，使用fallback策略")
        print(f"⚠️  警告: 没有参数组合满足 Sharpe > {sharpe_threshold} 且換手率 {turnover_min}~{turnover_max} 的条件")
        print("回退到初始最佳夏普比率参数")
        # Fallback to best sharpe parameter
        best_robust_params = best_params
        sensitivity_report = {
            "warning": "No parameters met robustness criteria (Sharpe + Turnover) - FALLBACK USED",
            "robustness_score": 0,
            "fallback_used": True,
...
def run_oos_test(
    ...
    **kwargs
    ...
    try:
        ...
        holding_days = int(best_params.get("holding_days", rebalance_freq_days))  # Default to rebalance_freq_days if not provided
...
def run_walkforward_with_sensitivity(
    ...
    Args:
        ...
        turnover_min: Minimum turnover rate (default 0.01)
        turnover_max: Maximum turnover rate (default 0.7)
    ...
    print(f"  換手率範圍: {turnover_min} ~ {turnover_max}")
...
if __name__ == "__main__":
    ...
    PARAM_GRID = {
        "lookback_period": range(1, 450, 3),        # Momentum lookback: 20-200 days, step 10
        "long_short_percentile": range(10, 20, 10),     # Top/bottom percentile: 10%-40%, step 5%
        "leverage": range(1, 2, 1),                   # Leverage: 1x
        "rebalance_freq_days": range(1, 2, 1),         # Rebalance frequency: 7-42 days, step 7 (ensures 2-13 rebalances in 90-day test window)
        "holding_days": range(1, 2, 1)               # Holding period: must match or relate to rebalance_freq_days for RotatingPositionStrategy
    }
    ...
    COMMISSION_BPS = 2       # 0.2% commission
