...
class CVILLIQFactor:
    """
    Coefficient of Variation of Illiquidity (CVILLIQ) Factor.

    This factor measures the relative volatility of an asset's illiquidity.
    It is based on the idea that the unpredictability of illiquidity is a risk
    ...
    def __init__(self, factor_name: str = "CVILLIQ"):
        """
        Initializes the CVILLIQFactor.
    ...
    def calculate(
        ...
        """
        Calculate CVILLIQ factor values.
        ...
        for symbol, df in price_data_dict.items():
            ...
            try:
                ...
                # Shift by 1 to avoid lookahead bias, using past data to generate signals for the current bar
                factor_df[symbol] = cvilliq.shift(1).reindex(all_dates)

            except Exception as e:
                logger.error(f"Error calculating CVILLIQ for {symbol}: {e}", exc_info=True)
                continue
...
if __name__ == "__main__":
    print("=" * 70)
    print("Running CVILLIQ Factor Calculation Test")
    print("=" * 70)
